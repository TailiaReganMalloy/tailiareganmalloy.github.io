...
window.initSent = async function (sent, sel) {
  // DEBUG: Confirm script runs and selector works
  const debugDivByClass = document.querySelector('.sent.hamlet');
  const debugDivById = document.getElementById('sent-hamlet');
  console.log('[init-sent.js] Running initSent. .sent.hamlet:', debugDivByClass, ' #sent-hamlet:', debugDivById);
  var isHamlet = sent.class == 'hamlet';
  var isMobile = innerWidth < 900;

  // Try to select by id if present, else by class
  var sel = debugDivById
    ? d3.select('#sent-hamlet')
    : d3.select('.' + sent.class);
  sel.st({opacity: 0.5, marginBottom: isHamlet ? '' : 40});
  ...
  var tokenSel = divSel
    ...
    .text((d) => d.str)
    .st({cursor: 'default', pointerEvents: 'none'});
