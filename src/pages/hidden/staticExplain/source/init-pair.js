...
window.initPair = function (pair) {
  ...
  var input1Sel = inputRow
    ...
    .at({cols: 30, readonly: true, disabled: true});
  ...
  // Hide Update button for static version
  ...
  // Hide Reset button for static version
  var resetSel = optionSel
    .append('div.reset')
    .html('<span>↻</span> Reset')
    .st({display: 'none'});

  // Disable alternate sentence selection for static version
  if (pair.alts) {
